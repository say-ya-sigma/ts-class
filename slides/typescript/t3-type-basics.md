...
## number型：世界でも屈指のイカれた型（1/2）

### 「整数と小数の区別がない」という狂気

```typescript
// JavaScript/TypeScriptのnumberは全部「IEEE 754 倍精度浮動小数点数」
const int: number = 42;
const float: number = 3.14;

// 他の言語ではこう書くのが普通:
// Java: int a = 42; double b = 3.14;
// Rust: let a: i32 = 42; let b: f64 = 3.14;
// Go: var a int = 42; var b float64 = 3.14;
```

---

## number型：世界でも屈指のイカれた型（2/2）

**なぜイカれているか:**
- 整数型が存在しない（他の主要言語では常識）
- `number` 1つで整数・小数・無限大・NaN全部含む
- お金の計算を任せるのに抵抗があるエンジニアもいる

---

## number型の罠：整数の精度が失われる

```typescript
// 9007199254740991 以上で整数の精度が破壊される
console.log(9007199254740991 + 1);  // 9007199254740992 ✅
console.log(9007199254740992 + 1);  // 9007199254740992 ❌（同じ！）
console.log(9007199254740993);        // 9007199254740992 ❌（勝手に変わる）
```
...
## number型の罠：NaNとInfinity

```typescript
const a: number = NaN;       // ✅ コンパイル通る
const b: number = Infinity;  // ✅ コンパイル通る
const c: number = 1 / 0;     // ✅ Infinity（例外にならない）
```

**注意:** お金の計算では `bigint` や `decimal.js` などの専用ライブラリを使うことが多い
...
**ポイント:**
- `T` は型の変数（Type Variable）
- 関数を呼ぶ時に型が決まる
- あとで `Array<T>` やタプルで使うので覚えておいて
